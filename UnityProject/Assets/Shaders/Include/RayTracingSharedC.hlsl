...
Texture2DArray<float4> gIn_Textures;
...
#define  SAMPLEARRAY( index, coords ) SampleLevel( TEX_SAMPLER, float3( coords.xy, index ), coords.z )
...
{
    float2 texSize = float2(1024,1024); // Default size
...
#define CheckNonOpaqueTriangle( rayQuery, mipAndCone ) \
    { \
        ...
        float alpha = gIn_Textures.SAMPLEARRAY( baseTexture,coords ).w; \
        \
        if( alpha > 0.5 ) \
...
MaterialProps GetMaterialProps(GeometryProps geometryProps)
{
    ...
    float4 color = gIn_Textures.SAMPLEARRAY(baseTexture,coords);
